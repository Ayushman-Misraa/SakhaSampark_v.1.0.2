...
function initializePeer(username) {
    ...
    currentPeer.on('error', (err) => {
        ...
        showErrorMessage(`Connection error: ${err.type}`);
    });
}
        ...
        if (fileMessageEl) {
            ...
            if (downloadBtn) {
                ...
                downloadBtn.onclick = () => {
                    ...
                    a.href = url;
                    a.download = receivingFile.info.name;
                    document.body.appendChild(a);
                    a.click();
                    document.body.removeChild(a);

                    // Don't revoke URL if it's an image (we'll use it for preview)
                    if (!receivingFile.info.type.startsWith('image/')) {
                        URL.revokeObjectURL(url);
                    } else {
                        // Add image preview
                        addImagePreview(fileMessageEl, url, receivingFile.info.name);
                    }
                };
                    ...
                    if (downloadBtn) {
                        ...
                        downloadBtn.onclick = () => {
                            ...
                            if (!receivingFile.info.type.startsWith('image/')) {
                                ...
                                // Add image preview
                                addImagePreview(newFileMessageEl, url, receivingFile.info.name);
                            }
                        };
                    }
                }
    ...
    if (messageType === 'outgoing') {
        ...
        if (downloadBtn) {
            downloadBtn.onclick = () => {
                ...
                const url = URL.createObjectURL(fileData._file);
                const a = document.createElement('a');
                a.href = url;
